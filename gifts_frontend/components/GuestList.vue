<template>
    <v-card class="mx-auto" max-width="800">
        <v-list lines="two">
            ...
            <template
                v-for="(
                    [user, groups, guestLink, isBase64, userAvatar], key
                ) in specialUsers"
                v-else
                :key="key"
            >
                ...
                <v-divider
                    v-if="key !== specialUsers.length - 1"
                    inset
                ></v-divider>
            </template>
        </v-list>
...
const specialUsers = computed(() =>
    userStore.users
        ...
        .map((user) => {
            const guestInformation = userStore.guestInformations.find(
